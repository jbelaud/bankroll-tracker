...
import { normalizePublicHandle, normalizeXHandle, validPublicAvatarUrl, validPublicHandle, validPublicImageUrl, validXHandle } from "@/lib/public-tipster-profile";
...
export async function savePublicTipsterProfile(_state: PublicTipsterProfileState, form: FormData): Promise<PublicTipsterProfileState> {
  ...
  const publicBannerUrl = String(form.get("publicBannerUrl") ?? "").trim() || null;
  ...
  if (!validPublicImageUrl(publicBannerUrl)) return { error: "L’adresse de la bannière doit être une URL HTTPS valide." };

  try {
    await prisma.user.update({
      where: { id: user.id },
      data: { publicDisplayName, publicHandle, publicBio, publicAvatarUrl, publicBannerUrl, publicXHandle },
    });
  } catch (error) {
