...
Règles impératives :
- "date" : utilise exclusivement la date de PRISE DU PARI, jamais la date ou l'heure de l'événement. Elle peut être en bas du ticket ("10h02 - 22 juin 2026" → "2026-06-22") ou dans l'en-tête, près de la référence du pari ("14 Sept. 2026 • 18:32" → "2026-09-14"). Recopie cette dernière ligne exactement dans "ticketPlacedAtText" si elle est lisible. Sur un ticket PMU, la date "17 sept. 2026, 21:00" dans la carte de sélection est celle du MATCH : recopie-la exactement dans "eventStartText", jamais dans "date". Si le jour, l'année ou l'heure de l'événement sont illisibles, "eventStartText" vaut null ; n'invente pas une heure. Si la date complète de prise de pari n'est pas visible, mets "date": null : ne complète jamais avec la date actuelle ou celle du match.
...
- Lis d'abord le STATUT DU TICKET dans son en-tête, près du type de pari et de la cote. Recopie la ligne entière sans changement dans "ticketHeaderText" si elle est lisible, sinon null. Sur PMU, "Simple @ 2,63 • Gagné" impose "result": "Gagné", même si la ligne de sélection affiche seulement "Sélection gagnante : 2" ; "Simple @ 2,63 • En cours" impose "result": "En attente", même si un bouton "Cash Out" ou un "Gain potentiel" est visible. Le statut de chaque sélection ne remplace jamais le statut de l'en-tête. Ne déduis jamais le résultat depuis la date du match ou un chiffre de sélection. Relis l'en-tête avant de conclure "En attente".
- "ticketRef" : sur PMU, lis tous les chiffres après "Référence Du Pari" dans l'en-tête. Ne tronque pas le début ou la fin ; si un chiffre est illisible, mets null pour que l'utilisateur vérifie au lieu d'inventer une référence.
- Sur un ticket PMU 1N2, la ligne « 1N2 - Temps réglementaire: Équipe B » indique la sélection JOUÉE. La ligne suivante « Équipe A - Équipe B » indique seulement l'affiche. Dans "description" et "selections[0].label", conserve Équipe B et le marché visible, jamais une victoire inventée d'Équipe A. La ligne « Sélection gagnante : 2 » décrit le résultat du match ; elle ne remplace pas la sélection jouée, qui peut même être perdante. Relis la ligne de sélection avant de répondre.
