  ...
  it("skips an already analyzed screenshot and continues with the remaining batch", async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({
        ...
        scan: { usageId: "usage-2", rawExtraction: [], model: "test", promptVersion: "test-version", supportStatus: "TESTED", detectedBookmaker: "1xBet", detectionConfidence: 0.99, earnedReferralScans: 0 },
      }), { status: 200 }));
    ...
    expect(result.scans[0].promptVersion).toBe("test-version");
